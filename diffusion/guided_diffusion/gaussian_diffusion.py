...
class GaussianDiffusion:
    ...
    def ddim_sample(
        ...
        # print(x.shape, gt_keep_mask.shape)
    ...
    def ddim_sample_loop_known(
        ...
        indices = list(range(t.unique().item()))[::-1]
    ...
    def ddim_sample_loop_progressive(
        ...
        for i in indices:
            ...
            with th.no_grad():
                ...
                # if i == 420:
                #     th.save(out["sample"], '/content/drive/MyDrive/diff_repaint/data/x_t.pt')

                yield out
                img = out["sample"]

        print(img.shape)
        # th.save(img, '/content/drive/MyDrive/diff_repaint/data/x_T.pt')
        # viz.image(visualize(img.cpu()[0,0, ...]), opts=dict(caption="reversesample"))
        if use_resample == False:
            for i in indices:
                t = th.tensor([i] * shape[0], device=device)
                with th.no_grad():
                    # if i == 80:
                    #     th.save(img, '/content/drive/MyDrive/diff_repaint/data/x_t_predict.pt')
                    ...
                    # if i == 80:
                    #     th.save(img, '/content/drive/MyDrive/diff_repaint/data/x_t_predict_prime.pt')
                    yield out
                    img = out["sample"]
            ...
            times = get_schedule_jump(500, 2, 1, 1)
            ...
            for t_last, t_cur in time_pairs:
                ...
                t_last_t = th.tensor([t_last] * shape[0],  # pylint: disable=not-callable
                                     device=device)
                print(i, len(time_pairs))
                i += 1
                if t_cur < t_last:  # reverse
                    with th.no_grad():
                        image_before_step = img.clone()
                        out = self.ddim_sample(
                            model,
                            img,
                            t_last_t,
