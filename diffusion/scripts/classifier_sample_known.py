...
def main():
    ...
    tensor_mask = th.zeros((32, 1, 256, 256), dtype=th.float32).to(dist_util.dev())
    ...
    for img in datal:
        ...
        if args.class_cond:
            ...
            # for i in range(32):
            #     if (img[2][i] == 1):
            #         non_zero_coords = th.nonzero(img[3][i, 0]).to(dist_util.dev())
            #         # Step 3: Compute bounding box
            #         print(non_zero_coords.shape)
            #         print(img[3][i, 0].shape)
            #         y1, x1 = non_zero_coords.min(dim=0).values
            #         y2, x2 = non_zero_coords.max(dim=0).values
            #         tensor_mask[i, 0, y1:y2+1, x1:x2+1] = 1.0
            #     else: tensor_mask[i, 0, :, :] = 1.0
            
            model_kwargs['gt_keep_mask'] = tensor_mask
        ...
        np.savez('/content/drive/MyDrive/diff_repaint/data/samples_new/samples_all_' + str(i) + '.npz',
