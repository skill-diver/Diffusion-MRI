...
def main():
    ...
    def adjust_white_area(img, brain_mask_single_channel, target_area, kernel_size=3):
        ...
        # 将图像转换为uint8类型，为接下来的膨胀和腐蚀操作做准备
        img = img.astype(np.uint8)
        print("max_img", np.max(img))
        
        # 如果当前的白色区域面积小于目标面积，进行膨胀操作直到达到目标面积
        ...
        # 如果当前的白色区域面积大于目标面积，进行腐蚀操作直到达到目标面积
    ...
    if args.class_cond:
        ...
        # model_kwargs['gt_keep_mask'] = abs(tensor_mask - th.from_numpy(label_binary[0]).float().to(dist_util.dev())) * brain_mask_multi_channel
