...
def random_translate_diff(diff, img_shape, area):
    ...
    # 获取非零坐标，即肿瘤区域
    non_zero_coords = np.where(diff > 0)
    ...
    print("non_zero_coords[0]",non_zero_coords[0])
    print("non_zero_coords[1]",non_zero_coords[1])

    diff_height = max_y - min_y + 1
    diff_width = max_x - min_x + 1
    
    max_translate_y = np.max(img_shape[2]) - np.min(img_shape[2]) - diff_height
    max_translate_x = np.max(img_shape[3]) - np.min(img_shape[3]) - diff_width
    
    
    print("img_shape[3]",img_shape[3])
    print("img_shape[2]",img_shape[2])
    # 平移起始点 相当于看成矩形，但其实是圆形，考虑从最大平移距离的1/3开始平移
    low_x = np.min(img_shape[3])+(1/3)*diff_width
    high_x = int(np.min(img_shape[3]) + (1/2)*max_translate_x)
    high_x = max(high_x, low_x + 1)  # 确保high_x总是大于low_x
    x = 0 if max_translate_x <= 0 else np.random.randint(low_x, high_x)

    low_y = np.min(img_shape[2])+(1/3)*diff_height
    high_y = int(np.min(img_shape[2]) + (1/2)*max_translate_y)
    high_y = max(high_y, low_y + 1)  # 确保high_y总是大于low_y
    y = 0 if max_translate_y <= 0 else np.random.randint(low_y, high_y)
...
def main():
    ...
    for img in datal:
        ...
        if isinstance(img, dict):
            print("Keys in this dictionary:", img.keys())
        else:
            print("img is not a dictionary.")
        ...
        threshold = 0.15
        current_diff[current_diff > threshold] = 1
        current_diff[current_diff <= threshold] = 0
        
        # 将二值图像转换为uint8类型，并乘以255，这样我们可以使用OpenCV处理它
        current_diff_uint8 = (current_diff * 255).astype(np.uint8)

        # 使用OpenCV找到所有轮廓
        contours, hierarchy = cv2.findContours(current_diff_uint8, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)

        # 设置比例阈值
        ratio_threshold = 1
        valid_contours = []

        min_ratio = float('inf')  # 设置一个非常大的起始值
        best_contour = None
        ...
        # 创建一个空的图像用于存储结果
        result = np.zeros_like(current_diff_uint8)
